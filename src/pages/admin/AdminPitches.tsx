...
import {
  CheckCircle2, XCircle, ExternalLink, Search, FileText, Clock,
} from "lucide-react";
import { toast } from "sonner";
import {
  PageHeader, PageShell, SectionCard, StatCard,
...
export default function AdminPitches() {
  ...
  const load = async () => {
    ...
    const [pitchRes, profilesRes, studentRes] = await Promise.all([
      supabase
        .from("pitches")
        .select("id,title,one_liner,stage,funding_ask,status,created_at,user_id,thumbnail_url,rejection_reason")
