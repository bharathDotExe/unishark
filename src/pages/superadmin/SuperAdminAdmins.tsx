  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-display font-extrabold text-white">Manage Admins</h1>
          <p className="text-white/40 text-sm mt-0.5">Create, configure and revoke admin access</p>
        </div>
        <Button
          className="bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white rounded-xl border-0 shadow-lg shadow-purple-500/20"
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="pl-9 h-9 bg-white/[0.04] border-white/[0.08] text-white placeholder:text-white/30 rounded-xl focus-visible:border-white/20 focus-visible:ring-0 text-sm"
        />
      </div>

      {/* Admin List */}
      {loading ? (
        <div className="space-y-3">{Array(3).fill(0).map((_, i) => <Card key={i} className="h-24 bg-white/5 animate-pulse rounded-xl" />)}</div>
      ) : (
        <div className="space-y-3">
          {filtered.map(admin => (
            <Card key={admin.id} className={`p-5 border ${admin.status === "suspended" ? "border-red-500/20 bg-red-500/5 opacity-80" : "border-white/[0.08] bg-white/[0.03]"} hover:bg-white/[0.05] transition-all`}>
              <div className="flex items-start justify-between flex-wrap gap-4">
                <div className="flex items-center gap-3">
                  <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${admin.role === "superadmin" ? "bg-gradient-to-br from-yellow-400 to-orange-500" : "bg-gradient-to-br from-purple-500 to-indigo-600"}`}>
                    {admin.role === "superadmin" ? <Crown className="h-4 w-4 text-white" /> : <UserCog className="h-4 w-4 text-white" />}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className={`font-bold ${admin.status === "suspended" ? "text-red-400 line-through decoration-red-500/50" : "text-white"}`}>{admin.name}</p>
                    ...
                    <div className="flex items-center gap-3 mt-0.5">
                      <span className="text-xs text-white/40 flex items-center gap-1"><Mail className="h-3 w-3" />{admin.email}</span>
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {admin.permissions.includes("all") ? (
                        ...
                        <Badge key={p} className="bg-white/5 text-white/50 border border-white/[0.08] text-[10px] capitalize">{p}</Badge>
                      ))}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button size="sm" variant="ghost" className="rounded-xl text-white/40 hover:text-white hover:bg-white/10">
                          <MoreHorizontal className="h-4 w-4" />
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent className="bg-[#0f0f1e] border border-white/[0.08] rounded-xl text-white">
                        <DropdownMenuItem className="text-xs cursor-pointer text-white/70 hover:text-white">
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="bg-[#0f0f1e] border border-white/[0.08] text-white max-w-md rounded-2xl">
          <DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Admin full name"
                className="bg-white/[0.05] border-white/[0.1] text-white placeholder:text-white/30 rounded-xl focus-visible:ring-0" />
            </div>
            <div>
            ...
            <div>
              <label className="text-xs text-white/50 mb-2 block font-medium">Permissions</label>
              <div className="grid grid-cols-3 gap-2">
                {ALL_PERMISSIONS.map(p => (
                  <button key={p} onClick={() => togglePerm(p)}
                    className={`px-2 py-1.5 rounded-lg text-xs font-medium border transition-all capitalize ${newPerms.includes(p) ? "bg-purple-500/20 border-purple-500/40 text-purple-300" : "bg-white/5 border-white/[0.08] text-white/40 hover:text-white/70"}`}>
          ...
          <DialogFooter>
            <Button variant="ghost" className="text-white/50 hover:text-white" onClick={() => setShowAdd(false)}>
              <X className="h-4 w-4 mr-1" /> Cancel
            </Button>
            <Button className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white border-0 rounded-xl" onClick={addAdmin}>
