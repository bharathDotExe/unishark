  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-display font-extrabold text-white">All Pitches — Full Control</h1>
          <p className="text-white/40 text-sm mt-0.5">View, approve, reject or delete any pitch</p>
        </div>
      ...
      {loading ? (
        <div className="space-y-3">{Array(4).fill(0).map((_,i)=>(
          <Card key={i} className="p-5 border border-white/[0.08] bg-white/[0.03] animate-pulse"><div className="h-5 bg-white/10 rounded w-1/3 mb-2"/><div className="h-4 bg-white/10 rounded w-2/3"/></Card>
        ))}</div>
      ) : filtered.length === 0 ? (
        <Card className="p-10 border border-white/[0.08] bg-white/[0.03] text-center">
          <Filter className="h-10 w-10 text-white/20 mx-auto mb-3"/><p className="text-white/50">No pitches found</p>
        </Card>
      ) : (
        <div className="space-y-3">
          {filtered.map(p => (
            <Card key={p.id} className="p-5 border border-white/[0.08] bg-white/[0.03] hover:bg-white/[0.05] transition-all overflow-hidden">
              <div className="flex items-start justify-between flex-wrap gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="font-bold text-white">{p.title}</h3>
                    <Badge className={`border text-xs font-bold ${statusClass[p.status] || "bg-white/10 text-white/50 border-white/20"}`}>{p.status === "SUBMITTED" ? "PENDING" : p.status}</Badge>
                    {p.stage && <Badge className="bg-white/5 text-white/40 border border-white/10 text-xs">{p.stage}</Badge>}
                  </div>
                  {p.description && <p className="text-sm text-white/40 mt-1 line-clamp-1">{p.description}</p>}
                  <div className="flex flex-wrap gap-3 mt-2 text-xs text-white/30">
                    {p.funding_ask && <span>Ask: <span className="text-white/60 font-medium">{p.funding_ask}</span></span>}
                ...
                <div className="flex gap-2 shrink-0 flex-wrap">
                  <Button asChild variant="ghost" size="sm" className="rounded-xl text-white/50 hover:text-white border border-white/[0.08] text-xs">
                  ...
                  <Button size="sm" className="bg-white/5 hover:bg-red-500/20 text-white/30 hover:text-red-400 border border-white/[0.08] hover:border-red-500/30 rounded-xl text-xs" onClick={() => deletePitch(p.id)}>
              ...
              {rejectFor === p.id && (
                <div className="mt-4 p-4 bg-red-500/5 border border-red-500/20 rounded-xl space-y-3">
                  <Textarea placeholder="Rejection reason..." value={reason} onChange={e => setReason(e.target.value)} rows={2} className="bg-white/5 border-red-500/30 text-white placeholder:text-white/30 text-sm focus-visible:ring-0"/>
                  <div className="flex gap-2">
                    <Button size="sm" className="bg-red-500 hover:bg-red-600 text-white rounded-xl text-xs" onClick={() => reject(p.id)}>Confirm Reject</Button>
                    <Button size="sm" variant="ghost" className="rounded-xl text-xs text-white/50" onClick={() => { setRejectFor(null); setReason(""); }}>Cancel</Button>
                  </div>
                </div>
              )}
            </Card>
