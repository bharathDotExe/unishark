  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-display font-extrabold text-white">Legal & Compliance</h1>
          <p className="text-white/40 text-sm mt-0.5">Regulatory compliance tracking for UniShark platform</p>
        </div>
        <Button variant="ghost" size="sm" className="rounded-xl text-white/50 hover:text-white border border-white/[0.08] text-xs">
          <Download className="h-4 w-4 mr-1.5"/>Export Report
        </Button>
      </div>

      {/* Compliance Score */}
      <Card className="p-6 border border-white/[0.08] bg-white/[0.03] relative overflow-hidden">
        ...
        <div className="relative flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-white/40 text-sm mt-1">{counts.COMPLIANT} of {ITEMS.length} items compliant</p>
      ...
      <div className="space-y-3">
        {filtered.map(item => {
          ...
          return (
            <Card key={item.id} className={`border overflow-hidden transition-all ${item.status==="NON_COMPLIANT"?"border-red-500/30":"border-white/[0.08]"} bg-white/[0.03]`}>
                  ...
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex flex-wrap gap-3 mt-2 text-xs text-white/30">
                        <span>Owner: <span className="text-white/50 font-medium">{item.owner}</span></span>
                        <span>Last review: <span className="text-white/50">{item.lastReviewed}</span></span>
                        <span>Next: <span className={new Date(item.nextReview) < new Date() ? "text-red-400 font-bold" : "text-white/50"}>{item.nextReview}</span></span>
                      </div>
                    </div>
                  </div>
                  {expanded===item.id?<ChevronUp className="h-4 w-4 text-white/30 shrink-0"/>:<ChevronDown className="h-4 w-4 text-white/30 shrink-0"/>}
                </div>
              </div>
              {expanded===item.id&&(
                <div className="border-t border-white/[0.06] p-5 space-y-4 bg-white/[0.02]">
                  <p className="text-sm text-white/60 leading-relaxed">{item.description}</p>
                  {item.documents && (
                    <div>
                      <p className="text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Documents</p>
                      <div className="flex flex-wrap gap-2">
                        {item.documents.map(d=>(
                          <Button key={d} variant="ghost" size="sm" className="h-8 rounded-xl text-xs text-white/50 hover:text-white border border-white/[0.08] hover:bg-white/10">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className="bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs border border-white/[0.1]">
